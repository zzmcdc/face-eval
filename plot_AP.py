...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Plots AP curves on AFW/PASCAL faces')
    parser.add_argument('detfile',type=str, nargs="?",default="",help='Detection file')
    parser.add_argument('--dataset',default="PASCAL", help='Select the dataset (AWF,PASCAL)')
    parser.add_argument('--oldAnn', help='Use old annotations',action="store_true")
    parser.add_argument('--minw', type=int, default=30, help='Minimum wide of a detectiion')
    parser.add_argument('--minh', type=int, default=30, help='Minimum height of a detectiion')
    parser.add_argument('--nit', type=int, default=5, help='Number of iterations for the bounding box refinement')
    ...
    if args.dataset=="AFW":
        ...
        tsImages=getRecord(AFW(minw=minw,minh=minh,useOldAnn=args.oldAnn),10000)
    elif args.dataset=="PASCAL":
        baseFolder = "detections/PASCAL"
        #testsetpath="/esat/kochab/mmathias/faceData/Annotations_Face_PASCALLayout_large/images"
        tsImages=getRecord(PASCALfaces(minw=minw,minh=minh,useOldAnn=args.oldAnn),10000)
    else:
    ...
    for fn in glob.glob(os.path.join(baseFolder,"*")):
        ...
        if ff[0]=="Picasa": #and ff[1]=="cvs": 
            ...
            #we evaluated manually, selecting overlap threshold of 0.1 gives in this case the correct result
            ovr=0.1
        dets = loadDetections(fn)
        dets = filterdet(dets, minpix)
        color,label = getColorLabel(fn)
        r = evaluate_optim(tsImages,dets,label, color, point=is_point, iter=nit,ovr=ovr)
        res.append(r)
