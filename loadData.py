...
def loadDetectionsShen(fn):
    ...
    for idl,dd in enumerate(f["DetectionResults"]):
        for ff in dd[0][0]["faces"][0]:
            det.append([dd[0][0]["filename"][0][0].split("\\")[-1].split(".")[0],ff[4],ff[0],ff[1],ff[0]+ff[2],ff[1]+ff[3]])
    dets=sorted(det, key=itemgetter(1),reverse=True)
    return det    
...
def loadDetectionsCSV(fn):
    ...
    for idl,dd in enumerate(rd):
        det.append([dd[2].split(".")[0],1,int(dd[7]),int(dd[8]),int(dd[7])+int(dd[9]),int(dd[8])+int(dd[10])])
    dets=sorted(det, key=itemgetter(1),reverse=True)
    return det    
...
def loadDetectionsRamanan(fn):
    ...
    for i in range(n):
        ...
        if 0:
            im=util.myimread("/users/visics/mpederso/databases/afw/testimages/"+this_id+".jpg")
            pylab.clf()
            pylab.imshow(im)
