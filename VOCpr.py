        ...
        if maxovr>ovr:
            if dimg[detbb[0]]["bbox"][gt][5] == 0:
                if not(dimg[detbb[0]]["det"][gt]):
                    tp[idx]=1
                    dimg[detbb[0]]["det"][gt]=True
                    gtx=dimg[detbb[0]]["bbox"][gt][3]-dimg[detbb[0]]["bbox"][gt][1]
                    dtx=detbb[4]-detbb[2]
                    gty=dimg[detbb[0]]["bbox"][gt][2]-dimg[detbb[0]]["bbox"][gt][0]
                    dty=detbb[5]-detbb[3]
                    gtcx=(dimg[detbb[0]]["bbox"][gt][3]+dimg[detbb[0]]["bbox"][gt][1])/2.
                    dtcx=(detbb[4]+detbb[2])/2.
                    gtcy=(dimg[detbb[0]]["bbox"][gt][2]+dimg[detbb[0]]["bbox"][gt][0])/2.
...
def filterdet(det,minpix,scale=1.0):
    ...
    for idl,l in enumerate(det):
        ...
        if ((l[4]-l[2])>minpix) or ((l[5]-l[3])>minpix):
            ndet.append([l[0],l[1],l[2]*scale,l[3]*scale,l[4]*scale,l[5]*scale])
...
def transf_dets(dets,tx,ty,sx,sy):
    ...
    for el in dets:
        ...
        dets2.append([el[0],el[1],cx-w*sx,cy-h*sy,cx+w*sx,cy+h*sy])
    return dets2
...
def evaluate_optim(tsImages,dets, lab,color,iter=3,point=False,ovr=0.5):
    ...
    ttx=0;tty=0;tsx=1;tsy=1
    for l in range(iter):
        tp,fp,scr,tot,tx,ty,sx,sy=VOCprRecordOptim(tsImages,dets,show=False,ovr=ovr)
        #for each TP compute the mean translation (tx,ty) and mean scale (sx,sy)
        tx=numpy.mean(tx)
        ty=numpy.mean(ty)
        sx=numpy.mean(sx)
        sy=numpy.mean(sy)
    ...
    if point:
        lb3 ="%s "%(lab)
        plotid, = pylab.plot(rc[-1],pr[-1],color=color,label=lb3,marker="o", markersize=10 , zorder=50)
        ap1 = -1
    else:
        lb3 ="%s (AP %.02f)" %(lab, ap1*100)
        plotid, = pylab.plot(rc,pr,color=color,label=lb3,linewidth=5 , zorder=10)
